...
const folders = new Map([
  [2, "service_002_app_team_analytics"],
  [3, "service_003_franchise_automation"],
  [4, "service_004_franchise_analytics"],
  [5, "service_005_saas_crm"],
  [6, "service_006_wholesale_automation"],
  [7, "service_007_marketing_agency_internal_tools"],
  [8, "service_008_lawyer_analytics"],
  [9, "service_009_hospital_automation"],
  [10, "service_010_hospital_analytics"],
]);
...
const categoryAssets = {
  ...
  automation: {
    toolStack: "Activepieces, Zapier, Make 또는 로컬 스크립트 중 고객 권한에 맞는 조합",
    deliverables: [
      "실패 알림 설계",
      "재시도 규칙",
      "고객 계정/API 권한 체크리스트",
    ...
    steps: [
      "문의 수집 채널과 담당자 라우팅 기준을 정리한다.",
      "자동화 흐름을 만들고 실패 알림과 재시도 규칙을 연결한다.",
      "테스트 문의를 발송해 누락, 중복, 지연 여부를 확인한다.",
      "고객 계정 권한과 월간 유지보수 절차를 문서화한다.",
    ],
  },
  crm: {
...
<head>
  ...
  <style>
    :root { color-scheme: light; font-family: Arial, "Malgun Gothic", sans-serif; background: #f6f7f2; color: #172026; }
    body { margin: 0; }
    main { max-width: 1040px; margin: 0 auto; padding: 40px 20px 56px; }
    .hero { display: grid; gap: 24px; grid-template-columns: minmax(0, 1.3fr) minmax(280px, .7fr); align-items: stretch; }
    section, aside { background: #fff; border: 1px solid #d9ded6; border-radius: 8px; padding: 24px; box-shadow: 0 8px 24px rgba(23,32,38,.06); }
    ...
    .meta span { border: 1px solid #c9d1c4; border-radius: 999px; padding: 7px 10px; font-size: 13px; background: #f8faf6; }
...
const progressDetails = [{
  number: "001",
  serviceId: "svc-352",
  name: "SaaS 분석 대시보드 서비스",
  artifactDir: "대화방/service_factory/service_001_saas_analytics",
  testEvidence: ".omo/start-work/service_001_green.txt",
  httpEvidence: ".omo/start-work/service_001_http_summary.txt",
  browserEvidence: ".omo/start-work/service_001_edge.png",
...
for (const service of selected) {
  ...
  progressDetails.push({
    ...
    name: service.name,
    artifactDir: `대화방/service_factory/${folder}`,
    platform: service.platform,
    paymentSystem: service.paymentSystem,
    price: service.price,
    testEvidence: ".omo/start-work/service_002_010_green.txt",
    httpEvidence: ".omo/start-work/service_002_010_http_all.txt",
    browserEvidence: ".omo/start-work/service_002_010_edge_010.png",
    ledger: ".omo/start-work/ledger.jsonl",
  });
}
...
writeFileSync(path.join(root, "service_002_010_index.html"), `<!doctype html>
<html lang="ko">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>서비스 002-010 출시 키트</title><style>body{font-family:Arial,"Malgun Gothic",sans-serif;margin:0;background:#f6f7f2;color:#172026}main{max-width:960px;margin:auto;padding:32px 20px}h1{font-size:30px}li{margin:10px 0;line-height:1.6}a{color:#1f6f5b;font-weight:700}.note{background:#fff6ef;border-left:4px solid #b85b38;padding:14px;border-radius:6px}</style></head>
<body><main><h1>서비스 002-010 출시 키트</h1><p class="note">실제 결제는 사용자 소유 계정에서 발급한 링크로 교체해야 하며 수익을 보장하지 않습니다.</p><ol>${indexRows}</ol></main></body>
</html>
`, "utf8");

writeFileSync(path.join(root, "service_execution_progress.json"), `${JSON.stringify({
  plan: "plans/480_service_numbered_plan.md",
  startedAt: "2026-06-01",
  updatedAt: "2026-06-02",
  completedServices: progressDetails.map((service) => service.number),
  completedServiceDetails: progressDetails,
  nextService: "011",
  status: "active",
}, null, 2)}\n`, "utf8");
...
if (existsSync(boulderPath)) {
  ...
  if (work) work.current_service = "011";
